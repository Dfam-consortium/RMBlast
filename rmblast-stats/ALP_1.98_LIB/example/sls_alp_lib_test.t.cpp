...
int main()
{
	...
	try
	{
		...
		long **substitutionScoreMatrix=S;
		...
		GumbelCalculator_obj.initGapless(
		alphabetSize,
		...
		f_gapless<<GumbelCalculator_obj;

		f_gapless.close();

		//computing gapped Gumbel parameters
		long gapOpen1=11;//gap opening penalty for sequence #1
		long gapEpen1=1;//gap extension penalty for sequence #1
		long gapOpen2=20;//gap opening penalty for sequence #2
		long gapEpen2=2;//gap extension penalty for sequence #2
		bool insertions_after_deletions=true;//if true, then insertions after deletions are permitted
		double eps_lambda=0.001;//relative error for the parameter lambda
		double eps_K=0.005;//relative error for the parameter K
		max_time=3.0;//maximum allowed calculation time in seconds
		double max_mem=1000;//maximum allowed memory usage in Mb
		long randomSeed=123456;//randomizaton seed

		//The function "initGapped" is called for a gapped scoring scheme; it assigns a new set of Gumbel parameters to the same object
		...
		//save the generalized randomization parameters
		AlignmentEvaluer::gapped_computation_parameters_struct gapped_computation_parameters_tmp=GumbelCalculator_obj.gapped_computation_parameters();
		

		//outputting the gapped parameters
		string st_gapped="gapped_lib.out";
		ofstream f_gapped(st_gapped.data());
		if(!f_gapped)
		{
			throw error("Error - file "+st_gapped+" is not found\n",3);
		};

		//the "operator <<" is called for the object what results outputting the gapped Gumbel parameters into the file "gapped_lib.out"
		f_gapped<<GumbelCalculator_obj;

		f_gapped.close();


		//reading saved gapless parameters from the file
		ifstream fin_gapless(st_gapless.data());
		if(!fin_gapless)
		{
			throw error("Error - file "+st_gapless+" is not found\n",3);
		};

		//the "operator >>" is used to read parameters from the file "gapless_lib.out"
		fin_gapless>>GumbelCalculator_obj;

		fin_gapless.close();
		

		//tests for different functions of P-value/E-value computation
		long score=35;//pairwise alignment score
		double seqlen1=200;//length of sequence #1
		double seqlen2=300;//length of sequence #2
		double pvalue;//resulted P-value
		double pvalueErr;//P-value error
		double evalue;//resulted E-value
		double evalueErr;//E-value error

		//calculating P-value/E-value without errors
		GumbelCalculator_obj.calc(
		score,
		seqlen1,
		seqlen2,
		pvalue,
		...
		//outputing the P-value on the screen
		cout<<"Gapless P-value is "<<pvalue<<endl;

		//another example of P-value/E-value calculation without errors
		evalue=GumbelCalculator_obj.evalue(score,seqlen1,seqlen2);
		pvalue=GumbelCalculator_obj.pvalue(evalue);

		//outputing the P-value on the screen
		cout<<"Gapless P-value (the second test) is "<<pvalue<<endl;


		//reading gapped parameters
		ifstream fin_gapped(st_gapped.data());
		if(!fin_gapped)
		{
			throw error("Error - file "+st_gapped+" is not found\n",3);
		};

		//the "operator >>" is used to read parameters from the file "gapped_lib.out"
		fin_gapped>>GumbelCalculator_obj;

		fin_gapped.close();


		//calculating gapped P-values/E-values
		GumbelCalculator_obj.calc(
		score,
		seqlen1,
		seqlen2,
		...
		//outputting the P-value on the screen
		cout<<"\nGapped P-value (the first test) is "<<pvalue<<" with error "<<pvalueErr<<endl;


		//the user-defined parameters are read from the file "gapped_lib.out" saved before
		fin_gapped.open(st_gapped.data());
		if(!fin_gapped)
		{
			throw error("Error - file "+st_gapped+" is not found\n",3);
		};
		...
		AlignmentEvaluerParametersWithErrors user_param;

		double C,C_error,a,a_error,alpha,alpha_error,gapless_a,gapless_a_error,
			gapless_alpha,gapless_alpha_error,G;

		//manual assignment of the Gumbel parameters "user_param" (reading the parameters computed above)
		string st_tmp;
		getline(fin_gapped,st_tmp);

		fin_gapped>>
			user_param.d_lambda>>user_param.d_lambda_error>>
			user_param.d_k>>user_param.d_k_error>>
			C>>C_error>>a>>a_error>>
			user_param.d_a1>>user_param.d_a1_error>>
			user_param.d_a2>>user_param.d_a2_error>>
			user_param.d_sigma>>user_param.d_sigma_error>>
			alpha>>alpha_error>>
			user_param.d_alpha1>>user_param.d_alpha1_error>>
			user_param.d_alpha2>>user_param.d_alpha2_error>>
			gapless_a>>gapless_a_error>>gapless_alpha>>gapless_alpha_error>>G;

		fin_gapped.close();

		//the intercepts are calculated accordingly
		user_param.d_b1=2.0*G*(gapless_a-user_param.d_a1); 
		user_param.d_b1_error=2.0*G*sqrt(gapless_a_error*gapless_a_error+user_param.d_a1_error*user_param.d_a1_error); 
		user_param.d_beta1=2.0*G*(gapless_alpha-user_param.d_alpha1); 
		user_param.d_beta1_error=2.0*G*sqrt(gapless_alpha_error*gapless_alpha_error+user_param.d_alpha1_error*user_param.d_alpha1_error); 

		user_param.d_b2=2.0*G*(gapless_a-user_param.d_a2); 
		user_param.d_b2_error=2.0*G*sqrt(gapless_a_error*gapless_a_error+user_param.d_a2_error*user_param.d_a2_error); 
		user_param.d_beta2=2.0*G*(gapless_alpha-user_param.d_alpha2); 
		user_param.d_beta2_error=2.0*G*sqrt(gapless_alpha_error*gapless_alpha_error+user_param.d_alpha2_error*user_param.d_alpha2_error); 

		user_param.d_tau=2.0*G*(gapless_alpha-user_param.d_sigma);
		user_param.d_tau_error=2.0*G*sqrt(gapless_alpha_error*gapless_alpha_error+user_param.d_sigma_error*user_param.d_sigma_error);

		//the function "initParameters" is called and the "AlignmentEvaluer" object is initialized by the user-defined parameters
		GumbelCalculator_obj.initParameters(user_param);
		...
		//outputing the P-value on the screen
		//the error "pvalueErr" output here will be slightly different from the one calculated before
		//due to the different algorithm of error estimation used in the case of user-defined parameters
		cout<<"Gapped P-value (the second test) is "<<pvalue<<" with error "<<pvalueErr<<endl;



		//a structure with user-defined parameters without errors
		AlignmentEvaluerParameters user_param_without_errors;

		//copying the parameters from the structure "user_param"
		user_param_without_errors.d_a1=user_param.d_a1;
		user_param_without_errors.d_a2=user_param.d_a2;

		user_param_without_errors.d_b1=user_param.d_b1;
		user_param_without_errors.d_b2=user_param.d_b2;

		user_param_without_errors.d_alpha1=user_param.d_alpha1;
		user_param_without_errors.d_alpha2=user_param.d_alpha2;

		user_param_without_errors.d_beta1=user_param.d_beta1;
		user_param_without_errors.d_beta2=user_param.d_beta2;

		user_param_without_errors.d_k=user_param.d_k;
		user_param_without_errors.d_lambda=user_param.d_lambda;
		user_param_without_errors.d_sigma=user_param.d_sigma;
		user_param_without_errors.d_tau=user_param.d_tau;

		//initializing the object by user-defined parameters without errors
		GumbelCalculator_obj.initParameters(user_param_without_errors);
		...
		//outputting the P-value on the screen
		cout<<"Gapped P-value (the third test) is "<<pvalue<<endl;

		//tests for the auxiliary functions
		cout<<"\nTests for auxiliary functions\n";
		cout<<"Function \"area\" returns "<<

			GumbelCalculator_obj.area(
			score,
			seqlen1,
			seqlen2)
		...
		cout<<"Function \"evaluePerArea\" returns "<<

			GumbelCalculator_obj.evaluePerArea(
			score)

			<<endl;

		cout<<"Function \"evalue\" returns "<<

			GumbelCalculator_obj.evalue(
			score,
			seqlen1,
			seqlen2)
		...
		cout<<"Function \"pvalue\" returns "<<

			GumbelCalculator_obj.pvalue(
		...
		cout<<"Function \"bitScore\" returns "<<

			GumbelCalculator_obj.bitScore(
			score)

			<<endl;

		cout<<"Function \"isGood\" returns "<<

			GumbelCalculator_obj.isGood()

			<<endl;

		cout<<"Function \"parameters().lambda\" returns "<<

			GumbelCalculator_obj.parameters().lambda

			<<endl;


		cout<<"Function \"parameters().K\" returns "<<

			GumbelCalculator_obj.parameters().K

			<<endl;

		//reproducing the previous computation
		max_time=-1;//computation time is set to a negative value

		//clear randomization parameters (for a test)
		GumbelCalculator_obj.gapped_computation_parameters_clear();

		//restore the randomization parameters from the variable saved before
		GumbelCalculator_obj.set_gapped_computation_parameters(gapped_computation_parameters_tmp);
		...
		//outputing the P-value on the screen
		//please compare the result with the first gapped test
		cout<<"\nGapped P-value (the fourth test) is "<<pvalue<<" with error "<<pvalueErr<<endl;
